...
void Display()
{
	...
	{
		// 빛의 방향 설정.
		glm::vec3 light_dir_f(-1.f, -1.f, 0.f);
		light_dir_f = glm::normalize(light_dir_f);

		// Apply Camera Matrices.
		////// *** 현재 카메라 방향을 고려하기 위해 view transform 적용  ***
		//  light_dir는 방향을 나타내는 벡터이므로 이동(Translation)변환은 무시되도록 한다. (네 번째 요소 0.f으로 셋팅)
		light_dir_f = glm::vec3(view_matrix * glm::vec4(light_dir_f, 0.f));

		int light_dir_loc = glGetUniformLocation(s_program_id, "light_dir_f");
		glUniform3f(light_dir_loc, light_dir_f[0], light_dir_f[1], light_dir_f[2]);
	...
	// Spot Light 설정
	{
		glm::vec3 pos(30.f, 0.5f, 0.f);
		glm::vec3 dir(-30.f, 0.f, 0.f);
		dir = glm::normalize(dir);


		pos = glm::rotateY(pos, rotation_y);
		dir = glm::rotateY(dir, rotation_y);

		pos = glm::vec3(view_matrix * glm::vec4(pos, 1.f));
		dir = glm::vec3(view_matrix * glm::vec4(dir, 0.f));

		int pos_loc = glGetUniformLocation(s_program_id, "pos");
		glUniform3f(pos_loc, pos[0], pos[1], pos[2]);

		int dir_loc = glGetUniformLocation(s_program_id, "dir");
		...
		{
			// 빛의 방향 설정.
			glm::vec3 dir2(-1.f, -1.f, 0.f);
			dir2 = glm::normalize(dir2);

			// Apply Camera Matrices.
			////// *** 현재 카메라 방향을 고려하기 위해 view transform 적용  ***
			//  light_dir는 방향을 나타내는 벡터이므로 이동(Translation)변환은 무시되도록 한다. (네 번째 요소 0.f으로 셋팅)
			dir2 = glm::vec3(view_matrix * glm::vec4(dir2, 0.f));

			int dir_loc2 = glGetUniformLocation(s_program_id, "dir2");
			glUniform3f(dir_loc2, dir2[0], dir2[1], dir2[2]);
		}
	...
	// st1
	{
		...
		glm::mat4 st1_T = glm::scale(glm::vec3(0.15f, 0.15f, 0.15f));
		glUniformMatrix4fv(m_model_loc, 1, GL_FALSE,  glm::value_ptr(st1_T));
		Drawst1Model();
	}
	...
	// st2
	{
		...
		glm::mat4 st2_T = glm::scale(glm::vec3(0.15f, 0.15f, 0.15f));
		glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(st2_T));
	...
	// st3
	{
		...
		glm::mat4 st3_T = glm::scale(glm::vec3(0.15f, 0.15f, 0.15f)) * glm::rotate(rotation_y, glm::vec3(0.f, -1.f, 0.f));
		glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(st3_T));
		Drawst3Model();
	}
	...
	// st4
	{
		...
		glm::mat4 st4_T = glm::scale(glm::vec3(0.15f, 0.15f, 0.15f)) * glm::rotate(rotation_y, glm::vec3(0.f, -1.f, 0.f));
		glUniformMatrix4fv(m_model_loc, 1, GL_FALSE, glm::value_ptr(st4_T));
		Drawst4Model();
	}
...
void Timer(int value)
{
	...
	// Turn
	rotation_y += angular_speed / 60.f;;

	// Calculate Velocity
	glm::vec3 speed_v = glm::vec3(0.f, 0.f, g_car_speed);
	glm::vec3 velocity = glm::rotateY(speed_v, rotation_y);	// speed_v 를 y축을 기준으로 g_car_rotation_y 만큼 회전한다.
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'n':
	{
		if (night_mode==true) {
			...
			glUniform1i(glGetUniformLocation(s_program_id, "night_mode"), false);
		}
		else {
			night_mode = true;
			// 'shading_mode' 값으로 false을 설정.
			glUniform1i(glGetUniformLocation(s_program_id, "night_mode"), true);
